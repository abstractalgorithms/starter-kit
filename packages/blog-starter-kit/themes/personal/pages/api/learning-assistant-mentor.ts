...
export default async function handler(
	...
	const incoming = (req.body ?? {}) as Partial<AssistantRequest>;
	const query = typeof incoming.query === 'string' ? incoming.query.trim() : '';
	const posts = Array.isArray(incoming.posts)
		? incoming.posts
				...
				.map((post) => ({
					title: post.title,
					slug: post.slug,
					brief: typeof post.brief === 'string' ? post.brief : '',
					readTimeInMinutes: Number.isFinite(post.readTimeInMinutes) ? Number(post.readTimeInMinutes) : 0,
					views: Number.isFinite(post.views) ? Number(post.views) : 0,
	...
	const history = Array.isArray(incoming.history)
		...
		: [];
	const persona = typeof incoming.persona === 'string' ? incoming.persona : undefined;
	const memoryContext = typeof incoming.memoryContext === 'string' ? incoming.memoryContext : undefined;
	const learningContext =
		incoming.learningContext && typeof incoming.learningContext === 'object'
			? incoming.learningContext
			: undefined;
	if (!query?.trim()) {
	...
	try {
		...
		console.error('[learning-assistant-mentor] upstream fetch failed:', error);
		res.setHeader('Cache-Control', 'no-store');
