		...
		<div className="mb-12 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 overflow-hidden">
				...
				<div className="flex items-center gap-2 flex-shrink-0">
					...
					{(state === 'idle' || state === 'error') && (
						...
						>
							...
							{state === 'error' ? 'Retry' : 'Generate AI Path'}
						</button>
					)}
					{state === 'ready' && (
						<button
			...
			{state === 'idle' && (
				<div className="flex flex-col items-center justify-center py-12 gap-3 text-center px-6">
					<div className="w-12 h-12 rounded-full bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center mb-1">
						<svg className="w-6 h-6 text-purple-600 dark:text-purple-400" fill="currentColor" viewBox="0 0 20 20">
							<path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
						</svg>
					</div>
					<p className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">
						AI-curated reading order
					</p>
					<p className="text-xs text-neutral-500 dark:text-neutral-400 max-w-xs">
						Let AI analyse the {posts.length} posts in this series and build a structured roadmap — sorted from foundational to advanced.
					</p>
				</div>
			)}
